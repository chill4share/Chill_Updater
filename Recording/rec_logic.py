...
class VideoManagement:
    ...
    @staticmethod
    def convert_mp4_to_mp3(file, output_file=None, recording_id='N/A', mp3_profile=None):
        file = os.path.normpath(file)
        ...
        try:
            ...
            if mp3_profile and "Nâng cao 1" in mp3_profile:
                audio_filter = "atempo=0.92,asetrate=44100*1.2,aresample=44100"
                ffmpeg_params.extend(["-ar", "44100", "-af", audio_filter])
            elif mp3_profile and "Nâng cao 2" in mp3_profile:
                audio_filter = "atempo=0.93,asetrate=48000*1.3,aresample=48000"
                ffmpeg_params.extend(["-ar", "48000", "-af", audio_filter])
            else:
                ffmpeg_params.extend(["-ab", "128k"])
            
            run_ffmpeg(file, output_file, ffmpeg_params, recording_id=recording_id)
        except Exception as e:
            logger.error(f"Lỗi chuyển đổi MP3: {e}")

class TikTokRecorder:
    def __init__(self, user, cookies=None, **kwargs):
        self.user = user
        ...
        self.get_mp3_options_callback = kwargs.get('get_mp3_options_callback')

        self.client = TikTokLiveClient(unique_id=self.user)
        if cookies and isinstance(cookies, str):
            cookie_jar = {k.strip(): v for k, v in (item.split('=', 1) for item in cookies.split(';') if '=' in item)}
    ...
    def _detail_log(self, message):
        if hasattr(self, 'detail_log_callback') and callable(self.detail_log_callback):
        ...
        try:
            for line in iter(pipe.readline, ''):
                ...
                if is_progress_line:
                    ...
                    # Cập nhật giao diện mỗi 10 giây để tránh làm rối
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                async def get_room_info():
                    room_id = await self.client.web.fetch_room_id_from_api(self.user)
                    if not room_id: raise UserOfflineError("API không trả về Room ID.")
                    room_info = await self.client.web.fetch_room_info(room_id)
                    if room_info.get("status", 4) == 4: raise UserOfflineError("User offline.")
                    return room_info

                room_info = loop.run_until_complete(get_room_info())
                ...
                self.start_recording(room_info)
                ...
                self._update_status(Status.ERROR_USER_NOT_FOUND, Colors.RED)
                self._detail_log(f"{Status.ERROR_USER_NOT_FOUND}. Dừng theo dõi.")
                logger.warning(f"Dừng theo dõi @{self.user} do: {Status.ERROR_USER_NOT_FOUND}")
                break
            except AgeRestrictedError:
                self._update_status(Status.ERROR_AGE_RESTRICTED, Colors.RED)
                self._detail_log(f"{Status.ERROR_AGE_RESTRICTED}. Dừng theo dõi.")
                logger.warning(f"Dừng theo dõi @{self.user} do: {Status.ERROR_AGE_RESTRICTED}")
                break
            except Exception as e:
        ...
        final_status = self.handle_post_recording()
    ...
    def start_recording(self, room_info):
        stream_url = self._get_best_stream_url(room_info)
        if not stream_url:
            self._update_status(Status.ERROR_NO_STREAM_URL, Colors.RED)
            self._detail_log("Không thể lấy URL stream từ API.")
            return

        self._update_status(Status.RECORDING, Colors.RED)
        base_name = re.sub(r'[\\/*?:"<>|]', "", self.custom_filename) if self.custom_filename else f"TK_{self.user}_{time.strftime('%Y%m%d_%H%M%S')}"
        ...
        self._detail_log("Bắt đầu tiến trình ghi hình bằng FFmpeg...")
        ...
        command = [
            ffmpeg_path, '-i', stream_url, '-c', 'copy', 
            '-bsf:a', 'aac_adtstoasc', '-y', self.output_filepath
        ]
        ...
        try:
            ...
            log_thread = threading.Thread(target=self._log_subprocess_output, args=(self.process.stderr,))
            ...
            while self.process.poll() is None and not self.stop_event.is_set():
                if time.time() - last_check_time >= 10:
                    try:
                        ...
                        logger.warning(f"Không thể kiểm tra kích thước file: {e}")
                    last_check_time = time.time()
                time.sleep(1)
            ...
            if self.stop_event.is_set():
                if self.manual_stop_requested and not self.cancellation_requested:
                    ...
                    try:
                        ...
                        self._detail_log(f"FFmpeg không phản hồi lệnh dừng, buộc phải kết thúc. Lỗi: {e}")
                        self.process.kill()
                else:
                    self._detail_log("Đã nhận tín hiệu Hủy, kết thúc tiến trình ngay lập tức.")
                    self.process.kill()
    ...
    def handle_post_recording(self):
        ...
        if not self.output_filepath or not os.path.exists(self.output_filepath) or os.path.getsize(self.output_filepath) <= 1024:
            ...
            if callable(self.failure_callback):
                self.failure_callback(self.recording_id, self.user)
            return {'status': 'failed', 'filepath': self.output_filepath}

        if self.manual_stop_requested:
            self._update_status(Status.DONE_STOPPED, Colors.DARK_BLUE)
            if callable(self.success_callback):
                self.success_callback(self.recording_id, self.user)
            return {'status': 'success', 'filepath': self.output_filepath}
        
        self._update_status(Status.DONE_SUCCESS, Colors.GREEN)
        if callable(self.success_callback):
            self.success_callback(self.recording_id, self.user)
        return {'status': 'success', 'filepath': self.output_filepath}
...
from Utils.config import DOUYIN_CONFIG
...
class DouyinAPI:
    def __init__(self, cookies):
        ...
        self.http_client = DouyinHttpClient(cookies=cookies, custom_headers={"Referer": "https://live.douyin.com/"})
    ...
    def get_live_info(self, web_rid: str):
        ...
        except (ValueError, KeyError) as e: raise RecordingException("Lỗi phân tích dữ liệu API.")

class DouyinRecorder:
    def __init__(self, live_url, cookies, **kwargs):
        ...
        self.manual_stop_requested = False
        self.output_filepath = None
        self.user = f"Douyin_{self.web_rid}"
        logger.info(f"Khởi tạo Douyin recorder cho RID: {self.web_rid}")

    def _update_status(self, message, color, is_countdown=False):
        if callable(self.status_callback): self.status_callback(self.recording_id, message, color, is_countdown)
    
    def _detail_log(self, message):
        if callable(self.detail_log_callback): self.detail_log_callback(self.recording_id, f"[{time.strftime('%H:%M:%S')}] {message}")
    ...
    def run(self):
        ...
        try:
            ...
            if live_info.get("status") == 2:
                ...
                final_status = self.start_recording(live_info.get("live_url"))
            else:
            ...
            self._detail_log(f"Lỗi nghiêm trọng: {e}")
    ...
    def start_recording(self, live_url):
        if not live_url:
            ...
            return self.handle_post_recording()
